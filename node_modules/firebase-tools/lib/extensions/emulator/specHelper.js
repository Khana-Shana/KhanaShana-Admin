...
function getNodeVersion(resources) {
    ...
    if (functionNamesWithoutRuntime.length) {
        emulatorLogger_1.EmulatorLogger.logLabeled("WARN", "extensions", `No 'runtime' property found for the following functions, defaulting to nodejs8: ${functionNamesWithoutRuntime.join(", ")}`);
    }
    ...
    if (_.includes(versions, "nodejs6")) {
        emulatorLogger_1.EmulatorLogger.logLabeled("WARN", "extensions", "Node 6 is deprecated. We recommend upgrading to a newer version.");
        return "6";
    }
    return "8";
}
