...
function CustomerSupport() {
  ...
  const printingStar = (n) => {
    ...
    return arr.map((x) => {
      return <FaStar class="star" color="red" size={20} />;
    });
  };
    ...
    return myData.map((feedback) => {
      return (
        <tr>
          ...
          <td>
            {feedback.Date.toDate().getDate() +
              "-" +
              (feedback.Date.toDate().getMonth() + 1) +
              "-" +
              feedback.Date.toDate().getFullYear()}
          </td>
          <td>
            {!Number.isNaN(feedback.Rating) ? (
              printingStar(feedback.Rating)
            ) : (
              <div></div>
            )}
          </td>
          ...
          {feedback.Message.length < 50 ? (
            ...
            <td>
              ...
              <button
                onClick={() => viewFullMessage(feedback.Message)}
                className="dim ml2 bg-mid-gray viewFullMessage"
              >
                View Full Message
              </button>
            </td>
