...
function AdminMenu() {
  ...
  function handleselect() {
    if (selectall === false) {
      ...
      menu.map(
        (_, i) =>
          (document.getElementsByClassName("itemcheckbox")[i].checked = true)
      );
    } else {
      setselectall(false);
      menu.map(
        (_, i) =>
          (document.getElementsByClassName("itemcheckbox")[i].checked = false)
      );
    }
  }
  ...
  function uploadMenuImage(id, index) {
    try {
      ...
      var uploadTask = firebase_integration.storage
        .ref()
        .child("Menu/" + imageName)
        .put(image);
      uploadTask.on(
        "state_changed",
        function (snapshot) {
          ...
          setprogressbar([
            ...progressbar.slice(0, index),
            progress,
            ...progressbar.slice(index + 1),
          ]);
        },
        function (error) {
  ...
  function updateDatabase() {
    try {
      filteredmenu.map((item, i) => {
        firebase_integration.database
          .collection("Menu")
          .doc(item.DishID.toString())
          .update({
            DishID: item.DishID,
            Category: document.getElementsByClassName("categorydropdown")[i]
              .value,
  ...
  async function removeItems() {
    ...
    try {
        ...
        if (item.Category === "Daily Deal") {
          ...
          firebase_integration.database
            ...
            .then((docs) => {
              firebase_integration.storage
                .ref()
                .child("Deals/" + docs.data().ImageName)
                .delete()
                .catch(function (error) {
                  alert("An error occured. Please try again!");
                });
        ...
        //resets other categories
          ...
          firebase_integration.database
            ...
            .then((docs) => {
              firebase_integration.storage
                .ref()
                .child("Menu/" + docs.data().ImageName)
                .delete()
                .catch(function (error) {
                  alert("An error occured. Please try again!");
                });
      ...
      items_removed.length === 0 ? setfilteredmenu(menu) : setfilteredmenu([]);
      setselectall(false);
      setprogressbar([]);
    } catch (error) {
      alert("An error occured. Please try again!");
  ...
  function AddItem() {
    ...
    menu.length === 0 ? (newDishID = 0) : (newDishID += 1);
    try {
      firebase_integration.database
        .collection("Menu")
        .doc(newDishID.toString())
        .set({
          DishID: newDishID,
          Category: "All",
          Name: "Food Item",
              ...
              <tr key={x.ID}>
                ...
                <td>
                  <input
                    ...
                    type="text"
                    placeholder="Name"
                    value={filteredmenu[i].Name}
                    onChange={(e) => {
                      var changeditem = filteredmenu[i];
                      changeditem.Name = e.target.value;
                      setfilteredmenu([
                        ...filteredmenu.slice(0, i),
                        changeditem,
                        ...filteredmenu.slice(i + 1),
                      ]);
                    }}
                  />
                </td>
                <td>
                  ...
                  <select className="form-control categorydropdown">
                    <option value={categories.filter((y) => y === x.Category)}>
                      {categories.filter((y) => y === x.Category)}
                    </option>
                    {categories
                      .filter((y) => y != x.Category)
                      .map((z) => {
                ...
                <td>
                  <input
                    ...
                    type="text"
                    placeholder="Price"
                    value={filteredmenu[i].SalePrice}
                    onChange={(e) => {
                      var changeditem = filteredmenu[i];
                ...
                <td>
                  <input
                    ...
                    value={filteredmenu[i].Description}
                    onChange={(e) => {
                      var changeditem = filteredmenu[i];
                ...
                <td>
                  <input
                    ...
                    value={filteredmenu[i].PortionSize}
                    onChange={(e) => {
                      var changeditem = filteredmenu[i];
              ...
              <tr>
                <th scope="row">
                  <div className="custom-control custom-checkbox">
                    <input
                      type="checkbox"
                      className="custom-control-input itemcheckbox"
                      id={i + 1}
                    />
                ...
                <td style={{ color: "3C3C3C" }}>
                  {filteredmenu[i].Description}
                </td>
                <td style={{ color: "3C3C3C" }}>
                  {filteredmenu[i].PortionSize}
                </td>
                ...
                <td>
                  <button
                    type="button"
                    className="btn btn-outline-primary btn-sm m-0 waves-effect imagebutton"
                  >
                    Upload Image
                    <input
                      id={filteredmenu[i].DishID + " img"}
                      type="file"
                      accept="image/png, image/jpeg"
                      onChange={() =>
                        uploadMenuImage(filteredmenu[i].DishID + " img", i)
                      }
                    />
                  </button>
                </td>
                ...
                <td>
                  <div
                    className="progress uploaderbar"
                    style={{ marginTop: "7%" }}
                  >
                    <div
                      id={i.toString() + " uploader"}
      ...
      <div id="menubox" className="container">
        {editmode ? (
          ...
          <div className="row">
            <button
              type="button"
              className="btn btn-sm menubutton"
              onClick={() => AddItem()}
            >
              Add
            </button>
              ...
              <thead>
                {editmode === true ? (
                  <tr>
                    ...
                    <th style={{ color: "3C3C3C" }} scope="col">
                      ID
                    </th>
                    <th style={{ color: "3C3C3C" }} scope="col">
                      Name
